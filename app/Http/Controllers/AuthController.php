...
class AuthController extends Controller{
	...
	function login(Request $request){
		...
		if(filter_var($username, FILTER_VALIDATE_EMAIL)) {
			...
			Auth::attempt(['user' => $username, 'password' => $password]); //login with username
		}
        if (!Auth::check()) {
     ...
     * @return \Illuminate\Http\Response
     */
	function register_store(UsuariosRegisterRequest $request){
		...
		$user = $record = Usuarios::create($modeldata);
		$rec_id = $record->id;
		Auth::login($user);
		return $this->redirectIntended("/home", "Inicio de sesión completado");
	}
